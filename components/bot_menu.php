<section id="menu-bot" class="row fixed-bottom justify-content-center align-items-end p-0 pb-4 m-0 mt-2">
    <article class="col-2">
        <a href="menu.php"><img src="assets/img/frontend/menu/casa.png" alt="" class="img-fluid"></a>
    </article>
    <div class="col-2"></div>
    <article class="col-2">
        <img src="assets/img/frontend/menu/mapa.png" alt="" class="img-fluid">
    </article>
    <div class="col-2"></div>
    <article id="menu" class="col-2">
        <img  src="assets/img/frontend/menu/hamburger.png" alt="" class="img-fluid">
    </article>
</section>